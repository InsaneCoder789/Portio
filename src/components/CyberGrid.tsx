  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const node of hexNodes) {
        ...
        ctx.closePath();
        ctx.strokeStyle = `hsla(327, 100%, 64%, ${opacity * 0.3})`;
        ctx.lineWidth = 0.5;
        ctx.stroke();
        ...
        if (dist < 200) {
          ...
          ctx.fillStyle = `hsla(265, 100%, 75%, ${proximity * 1.2})`;
          ctx.fill();
        }
      }
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        const glowOpacity = p.opacity * (0.4 + pulse.value * 0.6) * breathe;

        // Particle glow
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.size + 4, 0, Math.PI * 2);
        ctx.fillStyle = `hsla(35, 100%, 60%, ${glowOpacity * 0.08})`;
        ctx.fill();

        // Particle core
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.size * 0.8, 0, Math.PI * 2);
        ctx.fillStyle = `hsla(327, 100%, 70%, ${glowOpacity})`;
        ctx.fill();
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < maxDist) {
            const lineOpacity = (1 - dist / maxDist) * 0.2 * (0.3 + pulse.value * 0.7);
            ctx.beginPath();
            ctx.moveTo(p.x, p.y);
            ctx.lineTo(p2.x, p2.y);
            ctx.strokeStyle = `hsla(265, 100%, 65%, ${lineOpacity})`;
            ctx.lineWidth = 0.4;
            ctx.stroke();
          }
        }
